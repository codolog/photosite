...
### Замечание по Node.js и Supabase

В логах сборки может появляться предупреждение: *«Node.js 18 and below are deprecated for @supabase/supabase-js»*. Чтобы его убрать, на Netlify должна использоваться Node.js 20 или новее. В проекте в `netlify.toml` задано `NODE_VERSION = "20"`; если деплой делался раньше с Node 18 — при следующем деплое после push обновлённого репозитория предупреждение должно исчезнуть. Подробнее: https://github.com/orgs/supabase/discussions/37217
